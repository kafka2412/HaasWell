...
def  main():
  ...
  installer = "\installer\HaasWell Installer.exe"
   
  if demo:
    installer = "\installer\HaasWell Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\HaasWell manual.pdf" 
  ]
  ...
  ZIPNAME = "HaasWell-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "HaasWell-v" + FULLVERSIONSTR + "-win-demo.zip"
